 ...
 */
function course_maker_get_svg( $args = array() ) {
	// Make sure $args are an array.
	if ( empty( $args ) ) {
		return __( 'Please define default parameters in the form of an array.', 'course-maker' );
	}

	// Define an icon.
	if ( false === array_key_exists( 'icon', $args ) ) {
		return __( 'Please define an SVG icon filename.', 'course-maker' );
	}
	 ...
	 *
	 * Example 2 with title and description: <?php echo course_maker_get_svg( array( 'icon' => 'arrow-right', 'title' => __( 'This is the title', 'textdomain' ), 'desc' => __( 'This is the description', 'textdomain' ) ) ); ?>
	 *
...
function course_maker_nav_menu_social_icons( $item_output, $item, $depth, $args ) {
	...
	if ( $menu_object ) {
		...
		$social_icons = course_maker_social_links_icons();
...
add_filter( 'walker_nav_menu_start_el', 'course_maker_nav_menu_social_icons', 10, 4 );
	...
	if ( 'primary' === $args->theme_location ) {
		foreach ( $item->classes as $value ) {
			if ( 'menu-item-has-children' === $value || 'page_item_has_children' === $value ) {
				$title = $title . ' ' . course_maker_get_svg( array( 'icon' => 'angle-down' ) );
			}
		}
	}
 ...
 */
function course_maker_social_links_icons() {
	 ...
	 */
	return apply_filters( 'course_maker_social_links_icons', $social_links_icons );
}
